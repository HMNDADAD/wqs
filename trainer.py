...
class MVectorTrainer(object):
    ...
    def __setup_model(self, input_size, is_train=False):
        ...
        if is_train:
            ...
            if self.configs.optimizer_conf.scheduler == 'WarmupCosineSchedulerLR':
    ...
    def __load_checkpoint(self, save_model_path, resume_model):
        ...
        if resume_model is not None or (os.path.exists(os.path.join(last_model_dir, 'model.pth'))
            ...
            state_dict = torch.load(os.path.join(resume_model, 'model.pth'),map_location='cpu')
            if isinstance(self.model, torch.nn.parallel.DistributedDataParallel):
                self.model.module.load_state_dict(state_dict)
            else:
                self.model.load_state_dict(state_dict)
            self.optimizer.load_state_dict(torch.load(os.path.join(resume_model, 'optimizer.pth'),map_location='cpu'))
            # 自动混合精度参数
